...
function GetMetadata(aSQLModel:TSQLModel):string;
var
  aMetaData:TMetaData;
  x,y:integer;
  FN:string;
...
begin
  ...
  begin
    ...
    if ( (Assigned(aSQLModel.Owner)) AND (rsoGetID_str in (aSQLModel.Owner AS TSQLRestServer).Options) ) then
    begin
...
begin
  ...
  begin
    ...
    begin
      ...
      FileName := IncludeTrailingPathDelimiter(ExtractFilePath(ParamStr(0))+WEBROOT)+UTF8ToString(FN);
...
begin
  ...
  // make metadata
  MetaData:=GetMetadata(aServer.Model);
end;
