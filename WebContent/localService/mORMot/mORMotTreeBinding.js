...
// Provides the mORMot model implementation of a tree binding
sap.ui.define(['jquery.sap.global', 
	 ...
	 *
	 * @class
	 * Tree binding implementation for mORMot models.
	 * The necessary property annotations, as well as accepted/default values are documented in the specification.
	 * 
	 * In addition to these hieararchy annotations, the mORMotTreeBinding also supports (cyclic) references between entities based on navigation properties.
	 ...
	 * }
	 * 
	 * In OperationMode.Server, the filtering on the mORMotTreeBinding is only supported with initial filters. 
	 ...
	 * 
	 * With OperationMode.Client and OperationMode.Auto, the mORMotTreeBinding also supports control filters.
	 ...
	 *                                           will be sent with every request. Filtering on the mORMotTreeBinding is only supported with initial filters.
	 ...
	 * 
	 * @public
	 * @alias sap.ui.model.mORMot.mORMotTreeBinding
	 * @extends sap.ui.model.TreeBinding
	 */
	var mORMotTreeBinding = TreeBinding.extend("sap.ui.model.mORMot.mORMotTreeBinding", /** @lends sap.ui.model.mORMot.mORMotTreeBinding.prototype */ {
		...
		constructor : function(oModel, sPath, oContext, aFilters, mParameters, aSorters){
			...
			if (this.sCountMode == CountMode.None) {
				jQuery.log.fatal("To use an mORMotTreeBinding at least one CountMode must be supported by the service!");
			}
	 ...
	 */
	mORMotTreeBinding.DRILLSTATES = {
		Collapsed: "collapsed",
		Expanded: "expanded",
		Leaf: "leaf"
	};
	
	mORMotTreeBinding.prototype._getNodeFilterParams = function (mParams) {
	 ...
	 * @private
	 */
	mORMotTreeBinding.prototype._loadSingleRootNodeByNavigationProperties = function (sNodeId, sRequestKey) {
		var that = this,
			sGroupId;
	 ...
	 * @public
	 */
	mORMotTreeBinding.prototype.getRootContexts = function(iStartIndex, iLength, iThreshold) {
	 ...
	 * @public
	 */
	mORMotTreeBinding.prototype.getNodeContexts = function(oContext, iStartIndex, iLength, iThreshold) {
	 ...
	 * If the mORMotTreeBinding is running with hierarchy annotations, a context with the property values "expanded" or "collapsed"
	 ...
	 * @public
	 */
	mORMotTreeBinding.prototype.hasChildren = function(oContext) {
		if (this.bHasTreeAnnotations) {
			if (!oContext) {
				return false;
			}
	 ...
	 * @public
	 */
	mORMotTreeBinding.prototype.getChildCount = function(oContext) {
		if (this.bHasTreeAnnotations) {
			var vHierarchyNode;
	 ...
	 * @private
	 */
	mORMotTreeBinding.prototype._getContextsForNodeId = function(sNodeId, iStartIndex, iLength, iThreshold, mRequestParameters) {
		var aContexts = [],
			sKey;
		...
		if (aMissingSections.length > 0) {
			...
			if (!this.bClientOperation) {
				...
				if (!this.oAllKeys && !this.mRequestHandles[mORMotTreeBinding.REQUEST_KEY_CLIENT]) {
	 ...
	 */
	mORMotTreeBinding.prototype._getCountForCollection = function () {
	 ...
	 * @private
	 */
	mORMotTreeBinding.prototype._getCountForNodeId = function(sNodeId, iStartIndex, iLength, iThreshold, mParameters) {
		var that = this,
			sGroupId;
	 ...
	 * @private
	 */
	mORMotTreeBinding.prototype._loadSubNodes = function(sNodeId, iStartIndex, iLength, iThreshold, aParams, mParameters, oRequestedSection) {
		var that = this,
			sGroupId,
	...
	mORMotTreeBinding.REQUEST_KEY_CLIENT = "_OPERATIONMODE_CLIENT_TREE_LOADING";
	 ...
	 */
	mORMotTreeBinding.prototype._loadCompleteTreeWithAnnotations = function (aURLParams) {
		var that = this;
		
		var sRequestKey = mORMotTreeBinding.REQUEST_KEY_CLIENT;
			...
			if (oData.results && oData.results.length > 0) {
				...
				for (var k = 0; k < oData.results.length; k++) {
					...
					if (mParentIds[sDataKey]) {
						jQuery.sap.log.warning("mORMotTreeBinding - Duplicate data entry for key: " + sDataKey + "!");
					}
	 ...
	 * @private
	 */
	mORMotTreeBinding.prototype.resetData = function(oContext, mParameters) {
		if (oContext) {
	 ...
	 * @public
	 */
	mORMotTreeBinding.prototype.refresh = function(bForceUpdate, sGroupId) {
	 ...
	 * @private
	 */
	mORMotTreeBinding.prototype._refresh = function(bForceUpdate, mChangedEntities, mEntityTypes) {
	 ...
	 * @return {sap.ui.model.odata.v2.mORMotTreeBinding} returns <code>this</code> to facilitate method chaining
	 * @public
	 */
	mORMotTreeBinding.prototype.filter = function(aFilters){
		...
		if (this.bClientOperation) {
			...
			jQuery.sap.log.warning("Filtering is ONLY possible if the mORMotTreeBinding is running in OperationMode.Client or " +
	 ...
	 */
	mORMotTreeBinding.prototype._applyFilter = function () {
		var that = this;
	...
	mORMotTreeBinding.prototype._filterRecursive = function (oNode, mKeys, fnFilterKey) {
	 ...
	 * @public
	 */
	mORMotTreeBinding.prototype.sort = function(aSorters, bReturnSuccess) {
	 ...
	 * @private
	 */
	mORMotTreeBinding.prototype._applySort = function() {
		var that = this,
			oContext;
	 ...
	 * @private
	 */
	mORMotTreeBinding.prototype.checkUpdate = function(bForceUpdate, mChangedEntities){
		var bChangeDetected = false;
		if (!bForceUpdate) {
	 ...
	 * @private
	 */
	mORMotTreeBinding.prototype._getNavPath = function(sPath) {
	 ...
	 * @private
	 */
	mORMotTreeBinding.prototype._processODataObject = function(oObject, sPath, sNavPath) {
		var aNavPath = [],
			that = this;
	 ...
	 * @private
	 */
	mORMotTreeBinding.prototype._hasTreeAnnotations = function() {
		...
		//List of all annotations that are required for the mORMotTreeBinding to work
	 ...
	 * metadata is loaded.
	 * 
	 * @returns {sap.ui.model.odata.v2.mORMotTreeBinding} The binding instance
	 * @public
	 */
	mORMotTreeBinding.prototype.initialize = function() {
	 ...
	 * @private
	 */
	mORMotTreeBinding.prototype._processSelectParameters = function () {
		if (this.mParameters) {
	 ...
	 * @private
	 */
	mORMotTreeBinding.prototype.getDownloadUrl = function(sFormat) {
		var aParams = [],
			sPath;
	 ...
	 * @protected
	 * @name sap.ui.model.odata.mORMotTreeBinding#setNumberOfExpandedLevels
	 * @function
	 */
	mORMotTreeBinding.prototype.setNumberOfExpandedLevels = function(iLevels) {
		iLevels = iLevels || 0;
		if (iLevels < 0) {
			jQuery.sap.log.warning("mORMotTreeBinding: numberOfExpandedLevels was set to 0. Negative values are prohibited.");
			iLevels = 0;
		}
	...
	/**
	 * Retrieves the currently set number of expanded levels from the Binding (commonly an mORMotTreeBinding).
	 * @protected
	 * @name sap.ui.model.odata.mORMotTreeBinding#getNumberOfExpandedLevels
	 * @function
	 * @returns {int} the number of expanded levels
	 */
	mORMotTreeBinding.prototype.getNumberOfExpandedLevels = function() {
	 ...
	 * @public
	 */
	mORMotTreeBinding.prototype.setRootLevel = function(iRootLevel) {
		iRootLevel = parseInt(iRootLevel || 0, 10);
		if (iRootLevel < 0) {
			jQuery.sap.log.warning("mORMotTreeBinding: rootLevels was set to 0. Negative values are prohibited.");
			iRootLevel = 0;
		}
	 ...
	 * @public
	 */
	mORMotTreeBinding.prototype.getRootLevel = function() {
	 ...
	 * @private
	 */
	mORMotTreeBinding.prototype._getEntityType = function(){
		var sResolvedPath = this.oModel.resolve(this.sPath, this.oContext);
	 ...
	 */
	mORMotTreeBinding.prototype.getFilterParams = function() {
		if (this.aFilters) {
